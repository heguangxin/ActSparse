...
def _parse_train_args(args: Optional[Dict[str, Any]] = None) -> _TRAIN_CLS:
    ...
    # hgx add: custom arg
    # r = _parse_args(parser, args) # (arg1, arg2, ...)
    # if len(r) > 5:
...
def get_train_args(args: Optional[Dict[str, Any]] = None) -> _TRAIN_CLS:
    ...
    add for custom args
    """
    custom_args_flag = False
    r = _parse_train_args(args)
    if len(r) > 5:
        model_args, data_args, training_args, finetuning_args, generating_args, custom_args = r
        print(f"[DEBUG-HGX] custom_args is {custom_args}") # list: ['--split_layers', '11,12']
        # exit()
        custom_args_flag = True
    else:
        model_args, data_args, training_args, finetuning_args, generating_args = _parse_train_args(args)

    if custom_args_flag:
        custom_args[1] = custom_args[1].replace(" ", "")
        finetuning_args.split_layers = list(map(int, custom_args[1].split(',')))
